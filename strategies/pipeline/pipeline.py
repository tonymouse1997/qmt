...
class MultiFactorPipeline:
    def __init__(self, data_feed: Any, threshold: float = 0.0):
        """
        初始化多因子预测流水线
    ...
    def train(self, 
        ...
        try:
            ...
            # 4. 准备训练数据
            X, y = self._prepare_training_data(processed_data, target_time)
    ...
    def _prepare_training_data(self, 
        ...
        y = next_day_target['close'] - tick_data['close']
    ...
    def update_market_condition(self, market_data: Dict[str, pd.DataFrame]):
        ...
        # 计算市场波动率
        volatility = tick_data['close'].rolling(20).std().iloc[-1]
        
        # 计算市场趋势
        trend = tick_data['close'].rolling(20).mean().pct_change().iloc[-1]
